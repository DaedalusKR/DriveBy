...
def get_dir(path):
    ...
    tree_structure = TreeWidgetItem()  # top tree struct init as selected path - when complete return to caller
    ...
    tree_structure.setText(1, str(get_dir_size(path)))

    #tree_structure.sortColumn(1)
...
def add_dirs_to_tree(path, tree_structure, top_path_size):  # go through each directory and add treewidget for each dir and file
    ...
    for top_dir_file in dir_list:
        ...
        tree_structure_dir = TreeWidgetItem()  # treewidget to be added to the top dir treewidget
        #tree_structure_dir.sortItems(1)
        ...
        if not os.path.isdir(top_dir_file) \
            ...
            tree_structure_dir.setText(1, str(os.path.getsize(os.path.join(path, top_dir_file))))
        ...
        if os.path.isdir(os.path.join(path, top_dir_file)) \
            ...
            for file in file_list:
                if not file.startswith('.') \
                    ...
                    tree_structure_dir.setText(1, str(get_dir_size(os.path.join(path, top_dir_file))))
