...
def get_dir(path):
    ...
    tree_structure.setText(1, str(convert_from_bytes(t_size)) + dir_measure(t_size))# str(convert_from_bytes(t_size)) + dir_measure(t_size))
...
def add_dirs_to_tree(path, tree_structure, top_path_size):  # go through each directory and add treewidget for each dir and file
    ...
    for top_dir_file in dir_list:
        ...
        if not os.path.isdir(top_dir_file) \
            ...
            tree_structure_dir.setText(1, str(convert_from_bytes(get_file_stats(os.path.join(path, top_dir_file)))) + file_measure(os.path.join(path, top_dir_file)))
